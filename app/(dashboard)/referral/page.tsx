...
import { Users, UserPlus, Share2, Copy, Check, Barcode, ClipboardList, TrendingUp, Search, Filter, X, RefreshCw, Calendar, Mail, Clock } from 'lucide-react';
...
export default function ReferralPage() {
  const [codes, setCodes]               = useState<TransformedCode[]>([]);
  ...
  // ✅ FIX: Only treat userId as an object if it actually IS one (not a raw string ID)
  const resolveUserObject = (userId: ReferralCode['userId']): TransformedCode['userId'] => {
    ...
    // userId is a raw string UUID — not populated, treat as null
    return null;
  };

  // ✅ FIX: Show email prefix when userId is not populated
  const getUserDisplayName = (code: TransformedCode): string => {
    if (!code.userId) {
      return code.email ? code.email.split('@')[0] : 'Unassigned';
    }
    ...
    if (last)          return last;
    return code.userId.email || code.email?.split('@')[0] || 'Unassigned';
  };

  // ✅ FIX: Show email initial when userId is not populated
  const getUserInitials = (code: TransformedCode): string => {
    if (!code.userId) {
      return code.email ? code.email.charAt(0).toUpperCase() : '?';
    }
    const first = code.userId.firstName?.trim();
    const last  = code.userId.lastName?.trim();
    ...
    return code.userId.email?.charAt(0).toUpperCase() || '?';
  };
  ...
  const fetchReferralCodes = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const transformed: TransformedCode[] = data.data.map((code) => ({
          id:        code.id,
          code:      code.code,
          email:     code.email,
          // ✅ KEY FIX: resolveUserObject checks typeof before treating as object
          userId:    resolveUserObject(code.userId),
        ...
        setTotalCodes(data.meta?.total || transformed.length);

        if (transformed.length === 0 && currentPage > 1) setCurrentPage(1);
      } else {
        throw new Error(data.message || 'Invalid response structure from server');
      }
    } catch (err: any) {
      const msg = err.message || 'Failed to fetch invitation codes';
      setError(msg);
  ...
  const filteredCodes = useMemo(() => {
    return codes.filter((code) => {
      ...
      const userEmail = code.userId?.email || code.email || '';
  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        <StatCard title="Total Codes"      value={totalCodes}           icon={<Barcode className="w-5 h-5" />}     subtitle="All generated" />
        <StatCard title="Active Codes"     value={availableCount}       icon={<UserPlus className="w-5 h-5" />}    subtitle="Ready to use" variant="primary" />
        <StatCard title="Used Codes"       value={usedCount}            icon={<Users className="w-5 h-5" />}       subtitle="Registered users" />
        <StatCard title="Conversion Rate"  value={`${conversionRate}%`} icon={<TrendingUp className="w-5 h-5" />} trend={parseFloat(conversionRate)} />
        {expiringCount > 0 && (
          <StatCard title="Expiring Soon"  value={expiringCount}        icon={<Clock className="w-5 h-5" />}       subtitle="Within 7 days" />
        )}
      </div>

      {/* Generate */}
      <div className="bg-gradient-to-br from-[#2a2a2a] to-[#232323] rounded-2xl border border-white/10 p-6 shadow-xl">
        <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
          ...
          >
            {isGenerating ? (
              ...
              <><UserPlus className="w-4 h-4" /> Generate Code</>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-[#2a2a2a] to-[#232323] rounded-2xl border border-white/10 p-6 shadow-xl">
        <div className="flex items-center gap-2 mb-4">
          ...
          {hasActiveFilters && (
            <button onClick={clearFilters} className="ml-auto px-3 py-1.5 text-xs bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-all flex items-center gap-1.5 border border-red-500/30">
        ...
        <div className="mt-4 pt-4 border-t border-[#333333]/50 flex items-center justify-between">
          <p className="text-sm text-gray-400">
            ...
            <span className="text-white font-semibold">{totalCodes}</span> codes
          </p>
          {meta && (
      ...
      {/* Table */}
      <div className="bg-gradient-to-br from-[#2a2a2a] to-[#232323] rounded-2xl border border-white/10 shadow-xl overflow-hidden">
                    ...
                    <tr key={code.id} className="group hover:bg-[#1f1f1f]/50 transition-all">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <div className="w-9 h-9 bg-gradient-to-br from-[#EF6B23]/20 to-[#E4782C]/20 rounded-full flex items-center justify-center border border-[#EF6B23]/20 group-hover:scale-110 transition-transform flex-shrink-0">
                            <span className="text-xs font-bold text-[#EF6B23]">{initials}</span>
                          </div>
                          <div>
                            <p className="text-white text-sm font-medium">{userName}</p>
                            {/* ✅ Show subtle label if user is not yet registered */}
                            {!code.userId && (
                              <p className="text-[10px] text-gray-500 mt-0.5">Not registered yet</p>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6 hidden xl:table-cell">
                        <span className={`px-3 py-1.5 rounded-full text-xs font-semibold inline-flex items-center gap-1.5 ${
                          ...
                          <div className={`w-2 h-2 rounded-full ${!code.isUsed ? 'bg-[#4AD991]' : 'bg-red-400'}`} />
                      ...
                      <td className="py-4 px-6 hidden 2xl:table-cell">
                        {code.expiresAt ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-400 text-xs">{formatDate(code.expiresAt)}</span>
                          </div>
                        ) : (
        ...
        {!isLoading && filteredCodes.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-[#626262] text-sm mb-6">
              {hasActiveFilters ? 'Try adjusting your filters or search terms' : 'Generate your first invitation code to get started'}
            </p>
            {hasActiveFilters ? (
              <button onClick={clearFilters} className="px-6 py-2.5 bg-[#EF6B23]/20 text-[#EF6B23] rounded-lg hover:bg-[#EF6B23]/30 transition-all font-semibold text-sm flex items-center gap-2 mx-auto">
                <X className="w-4 h-4" /> Clear Filters
              </button>
            ) : (
              <button onClick={generateNewCode} disabled={isGenerating} className="px-6 py-2.5 bg-gradient-to-r from-[#EF6B23] to-[#E4782C] text-white rounded-lg hover:shadow-lg hover:shadow-[#EF6B23]/25 transition-all font-semibold text-sm flex items-center gap-2 mx-auto disabled:opacity-50">
                <UserPlus className="w-4 h-4" /> Generate First Code
              </button>
            )}
          </div>
        )}
        ...
        {meta && meta.totalPages > 1 && !isLoading && (
          <div className="p-6 border-t border-[#333333]/50 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              {[
                { label: 'First',    action: () => setCurrentPage(1),                                   disabled: !meta.hasPrevious },
                { label: 'Previous', action: () => setCurrentPage(p => Math.max(1, p - 1)),             disabled: !meta.hasPrevious },
                { label: 'Next',     action: () => setCurrentPage(p => Math.min(meta.totalPages, p + 1)), disabled: !meta.hasNext },
                { label: 'Last',     action: () => setCurrentPage(meta.totalPages),                     disabled: !meta.hasNext },
      ...
      <div className="bg-gradient-to-br from-[#2a2a2a] to-[#232323] rounded-2xl border border-white/10 p-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          {[
            { step: '1. Generate Code',     desc: 'Create unique invitation codes with 15-day validity',          icon: <UserPlus className="w-5 h-5 text-[#EF6B23]" />,  bg: 'bg-[#EF6B23]/20',  border: 'hover:border-[#EF6B23]/30' },
            { step: '2. Share Code',         desc: 'Copy and share codes with new users via email or chat',        icon: <Copy className="w-5 h-5 text-[#4AD991]" />,      bg: 'bg-[#4AD991]/20',  border: 'hover:border-[#4AD991]/30' },
            { step: '3. User Registration',  desc: 'Code is validated and marked as used during signup',           icon: <Users className="w-5 h-5 text-red-400" />,        bg: 'bg-red-500/20',     border: 'hover:border-red-500/30'   },
          ].map(({ step, desc, icon, bg, border }) => (
            <div key={step} className={`flex items-start gap-3 p-4 rounded-xl bg-[#151515]/50 border border-[#333333]/50 ${border} transition-all`}>
              <div className={`w-10 h-10 ${bg} rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5`}>
                {icon}
              </div>
              <div>
                <h4 className="font-semibold text-white mb-1">{step}</h4>
